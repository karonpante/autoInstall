...
class NetworkFile(TextFile):
    ...
    def __init__(self, location='/etc/sysconfig/', filename='network', content=''):
    ...
    def set_xml_content(self, general_element):
        ...
        hostname_configuration = 'HOSTNAME={0}'.format(hostname.text)

        # Read template file
        fh_template = FileHandler(path.join(exec_path, 'templates/network'))
